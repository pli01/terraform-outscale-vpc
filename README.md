...
- copy `config-sample.yml` (or on of the files in config-sample directory) to `config.yml` (default `config_file` from variables.tf) and modify to your needs
...
- generate `backend.tf` from `backend.tf.erb`
...
- optional: define volumes list, root volume, or multiple extra volumes:w
  ...
  cloud_init:
    ...
    cloudinit_web: &cloudinit_web              # define a cloud_init anchor for web instances
      ...
      - filename: config-script-demo.sh
        content_type: text/x-shellscript
        templatefile: cloud-init/config-script-demo.sh.tpl # templatefile is located in ./. Can use ${var} in it and define var in parameters map
