...
resource "outscale_volumes_link" "volumes_link" {
  #  for_each = contains(keys(var.value), "volumes") ? { for k, v in var.value.volumes : v.name => v if v.name != "root" } : {}
  for_each = { for k, v in local.volume_list : k => v if v.name != "root" }
